...
begin
  ...
  try
    VQry.ExecSQL('update tblvendas set quantidade_ven, preco_ven = :nome where (codigo_ven = :codigo)', [AVendasModel.Quantidade, AVendasModel.Codigo]);
    Result := True;
  finally
    VQry.Free;
  end;
...
begin
  VQry := FConexao.CriarQuery();
  try
    VQry.ExecSQL('insert into tblVenda (codigo_ven, quantidade_ven, preco_ven) values (:codigo, :nome)', [AVendasModel.Codigo, AVendasModel.Quantidade]);
    Result := True;
  finally
    VQry.Free;
  end;
